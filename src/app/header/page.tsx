...
export default function Header() {
  ...
  const [flagHover, setFlagHover] = useState(() => ({
    ...
    ukrHover: false,
    hover: false,
    flag: searchParams.get('lang'), // or ukr
  }))
  console.log(router)
    ...
    <header className="z-10 fixed left-[3.5rem] right-[3.5rem] top-[0rem] bottom-[calc(100vh-6rem)] flex py-[1.5rem] justify-between items-center self-stretch bg-background">
      ...
      {flagHover.hover ? (
        <>
          ...
          <div
            className="flex w-[11.9375rem] flex-col items-start border-[#8F5E00] border-[1px] border-solid shadow-xl shadow-[rgba(15,42,81,0.08)] absolute right-0 top-[1.25rem] cursor-pointer"
            onMouseLeave={() =>
